...
if os.path.exists(sys.argv[2]):
    ...
    for line in fp:
        ...
        celeb_labels[sline[0]] = np.array(label, dtype=np.float64)
    fp.close()
else:
    print("invalid path for attributes!")
    exit()

direct = list(os.listdir(sys.argv[1]))
labels_to_save = []
feats_to_save = []
count = 0
...
n_clusters = 18
for i, fname in enumerate(direct):
    if i == 6000:
        break
    print(count)
    label = celeb_labels[fname]
    ...
    feats_to_save.append(query_M.flatten().detach().cpu().numpy())
    count += 1

np.savez('ris_celeba_64x64_feats.npz', feats=np.array(feats_to_save), labels=np.array(labels_to_save))
