...
class Trainer(object):
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            c_loss = criterion(labels_predict, labels_real_plain.long())
            self.reset_grad()
            c_loss.backward()
    ...
    def build_model(self):
        ...
        self.G = unet().cuda()
        """
        for param in self.G.encoder.parameters():
            param.requires_grad = False
        for param in self.G.encoder1.parameters():
            param.requires_grad = False
        for param in self.G.encoder2.parameters():
            param.requires_grad = False
        """
        if self.parallel:
