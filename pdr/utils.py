...
def save_images(out_fold, imgs, prefix='', suffix='', sep_folder=True, ext='.png', img_name=None):
    if sep_folder:
        out_fold = os.path.join(out_fold, suffix)
    xmkdir(out_fold)
    prefix = prefix + '_' if prefix else ''
    ...
    for i, img in enumerate(imgs):
        ...
        if not img_name:
            ...
            cv2.imwrite(os.path.join(out_fold, prefix+img_name[i]+suffix+ext), im_out)
