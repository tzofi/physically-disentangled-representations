...
class Encoder(nn.Module):
    def __init__(self, cin, cout, nf=64, activation=nn.Tanh):
        ...
        network = [
            ...
            nn.ReLU(inplace=True)]
        self.conv = nn.Conv2d(nf*8, cout, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, input):
        ...
        return output.reshape(input.size(0),-1), features
...
class EDDeconv(nn.Module):
    def __init__(self, cin, cout, zdim=128, nf=64, activation=nn.Tanh):
        ...
        if activation is not None:
            network_up += [activation()]
        self.network_down = unet_encoder() #nn.Sequential(*network_down)
        #self.network_down = nn.Sequential(*network_down)
        self.conv = nn.Conv2d(256, 256, kernel_size=4, stride=1, padding=0, bias=False)
        self.network_up = nn.Sequential(*network_up)

    def forward(self, input):
        _,_,_,_,features = self.network_down(input)
        #features = self.network_down(input)
        feats = self.conv(features)
        output = self.network_up(feats)
        return output, F.normalize(feats)
