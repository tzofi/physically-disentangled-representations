...
class PDR():
    def __init__(self, cfgs):
        ...
        self.netD = networks.EDDeconv(cin=3, cout=1, nf=64, zdim=256, activation=None).to(device)
        self.netA = networks.EDDeconv(cin=3, cout=3, nf=64, zdim=256).to(device)
        self.netL = networks.Encoder(cin=3, cout=4, nf=32).to(device)
        self.netV = networks.Encoder(cin=3, cout=6, nf=32).to(device)
    ...
    def get_param_count(self):
        d = sum(p.numel() for p in self.netD.parameters() if p.requires_grad)
        a = sum(p.numel() for p in self.netA.parameters() if p.requires_grad)
        l = sum(p.numel() for p in self.netL.parameters() if p.requires_grad)
        v = sum(p.numel() for p in self.netV.parameters() if p.requires_grad)
        print("Depth: {}, Albedo: {}, Light: {}, View: {}".format(d,a,l,v))
    ...
    def get_features(self, input):
        ...
        _, depth = self.netD(self.input_im)
        _, albedo = self.netA(self.input_im)  # Bx3xHxW
        _, light = self.netL(self.input_im)
        _, view = self.netV(self.input_im)
        return depth, albedo, light, view

    def forward(self, input, cyclic=True):
        """Feedforward once."""
        if self.load_gt_depth:
            input, depth_gt = input
        self.input_im = input.to(self.device) *2.-1.
        ...
        self.canon_depth_raw, depth_feats = self.netD(self.input_im)
        ...
        self.canon_albedo, albedo_feats = self.netA(self.input_im)  # Bx3xHxW
        ...
        ## predict lighting
        canon_light, light_feats = self.netL(self.input_im)
        canon_light = canon_light.repeat(2,1)  # Bx4
        ...
        self.view, view_feats = self.netV(self.input_im)
        ...
        light_or_cam = None
        if cyclic:
            # Randomly choose light or view to change (for re-rendering)
            # => 0 --> change lighting
            # => 1 --> change view
            # For light only, we comment the following line to always be 0
            light_or_cam = torch.randint(low=0, high=2, size=(1,)) #random.randint(0, 1)
            ...
            if light_or_cam == 0:
                # Augmenting Light
                la = ((torch.rand((batch_size*2, 1)) * 2*.5) - .5).to(self.device)
                lb = ((torch.rand((batch_size*2, 1)) * 2*.5) - .5).to(self.device)
                ld = ((0.785398*torch.rand((batch_size*2, 3))) - 0.785398/2).to(self.device)

                # Update Ambient Lighting
                canon_light_a = self.canon_light_a + la

                # Update Diffuse Lighting
                canon_light_b = self.canon_light_b + lb

                # Update Lighting Direction
                canon_light_d = self.canon_light_d + ld
            elif light_or_cam == 1:
                # Augmenting Camera View
                # normal
                pitch = (0.785398/2*torch.rand((batch_size*2, 1))) - 0.785398/4
                yaw = (0.785398*torch.rand((batch_size*2, 1))) - 0.785398/2
                # stronger
                #pitch = (0.785398*torch.rand((batch_size*2, 1))) - 0.785398/2
                #yaw = (0.785398*2*torch.rand((batch_size*2, 1))) - 0.785398
                other = torch.zeros((batch_size*2, 4))
                view_change = torch.cat([pitch, yaw, other], dim=1).to(self.device)
                """
                yaw = 0
                if light_or_cam == 0:
                    yaw = torch.ones((batch_size*2, 1)) * (0.785398)
                else:
                    yaw = torch.ones((batch_size*2, 1)) * (-0.785398)
                other_1 = torch.zeros((batch_size*2, 1))
                other = torch.zeros((batch_size*2, 4))
                view_change = torch.cat([other_1, yaw, other], dim=1).to(self.device)
                """
                view = self.view + view_change

            ## shading
            self.canon_normal = self.renderer.get_normal_from_depth(self.canon_depth)
            self.canon_diffuse_shading = (self.canon_normal * canon_light_d.view(-1,1,1,3)).sum(3).clamp(min=0).unsqueeze(1)
            canon_shading = canon_light_a.view(-1,1,1,1) + canon_light_b.view(-1,1,1,1)*self.canon_diffuse_shading
            ...
            _, depth = self.netD(self.recon_im[:b])
            _, albedo = self.netA(self.recon_im[:b])
            _, light = self.netL(self.recon_im[:b])
            _, view = self.netV(self.recon_im[:b])
            cycle_feats = [depth, albedo, light, view]


        """
        ## predict lighting
        canon_light, light_feats = self.netL(self.input_im)
        print("Light Original: {}, Shape: {}".format(canon_light, canon_light.shape))
        canon_light = canon_light.repeat(2,1)  # Bx4
        print("Light Repeat: {}, Shape: {}".format(canon_light, canon_light.shape))
        self.canon_light_a = self.amb_light_rescaler(canon_light[:,:1])  # ambience term
        #self.canon_light_a[0][0] = 1.0
        #self.canon_light_a[1][0] = 1.0
        print("Ambient Light: {}, Shape: {}".format(self.canon_light_a, self.canon_light_a.shape))
        self.canon_light_b = self.diff_light_rescaler(canon_light[:,1:2])  # diffuse term
        print("Diffuse Light: {}, Shape: {}".format(self.canon_light_b, self.canon_light_b.shape))
        canon_light_dxy = canon_light[:,2:]
        self.canon_light_d = torch.cat([canon_light_dxy, torch.ones(b*2,1).to(self.input_im.device)], 1)
        self.canon_light_d = self.canon_light_d / ((self.canon_light_d**2).sum(1, keepdim=True))**0.5  # diffuse light direction
        self.canon_light_d[1][2] +=  7.85398
        print("Light: {}, Shape: {}".format(self.canon_light_d, self.canon_light_d.shape))
        """
        ...
        ## predict viewpoint transformation
        self.view, view_feats = self.netV(self.input_im)
        print("Output: {}, Shape: {}".format(self.view, self.view.shape))
        self.view = self.view.repeat(2,1)
        print("Repeat Output: {}, Shape: {}".format(self.view, self.view.shape))
        self.view = torch.cat([
            self.view[:,:3] *math.pi/180 *self.xyz_rotation_range,
            self.view[:,3:5] *self.xy_translation_range,
            self.view[:,5:] *self.z_translation_range], 1)
        print("Final Output: {}, Shape: {}".format(self.view, self.view.shape))
        #self.view[0][0] -= 0.785398/2
        """
        ...
        if self.load_gt_depth:
            ...
            print("Depth SIE Masked: {}, NoErr Masked: {}".format(metrics['SIE_masked'],metrics['NorErr_masked']))

        if not cyclic:
            return metrics, [depth_feats, albedo_feats, light_feats, view_feats]
        if cyclic:
            return metrics, [depth_feats, albedo_feats, light_feats, view_feats], cycle_feats, light_or_cam
    ...
    def save_results(self, save_dir, img_name):
        b, c, h, w = self.input_im.shape
        ...
        utils.save_images(save_dir, recon_im, suffix='recon_image', sep_folder=sep_folder, img_name=img_name)
